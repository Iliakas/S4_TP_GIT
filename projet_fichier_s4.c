...
void savePatientToFile(const pat *p, const char *filepath) {
    if (p == NULL || filepath == NULL || *filepath == '\0') {
        fprintf(stderr, "Erreur: Données patient ou chemin de fichier invalide pour la sauvegarde.\n");
        return;
    }
    ...
    if (file == NULL) {
        ...
        fprintf(stderr, "Impossible de sauvegarder le patient '%s %s' dans %s\n", p->prenom, p->nom, filepath);
        return;
    }

    fprintf(file, "--- Nouveau Patient ---\n");
    fprintf(file, "NOM: %s\n", p->nom);
    fprintf(file, "PRENOM: %s\n", p->prenom);
    fprintf(file, "SEXE: %s\n", p->sexe);

    if (p->maladie != NULL) {
        fprintf(file, "MALADIE_LIEE: %s\n", p->maladie->nom);
    } else {
        fprintf(file, "MALADIE_LIEE: Aucune\n");
    }

    fprintf(file, "--- FIN PATIENT ---\n\n");

    if (fclose(file) != 0) {
         perror("Erreur lors de la fermeture du fichier patients après écriture");
    } else {
        printf("Patient '%s %s' sauvegardé dans le fichier %s\n", p->prenom, p->nom, filepath);
    }
}
...
int main() {
    ...
    d date_recherche;
    char patientFilePath[PATH_MAX];

    printf("Veuillez saisir le chemin complet du fichier où stocker les patients (ex: /home/utilisateur/Desktop/patients.txt ou C:\\Users\\utilisateur\\Desktop\\patients.txt):\n");
    fgets(patientFilePath, PATH_MAX, stdin);
    patientFilePath[strcspn(patientFilePath, "\n")] = 0;
                 ...
                 do {
                        ...
                        switch (sous_choix) {
                            case 1: // Ajout de patient
                                ...
                                if (nvPatient != NULL) {
                                    // Demander et lier la maladie AVANT la sauvegarde
                                    printf("Saisir le nom de la maladie a lier (ou laisser vide si aucune) : ");
                                    fgets(maladie_nom, 256, stdin);
                                    maladie_nom[strcspn(maladie_nom, "\n")] = 0;

                                    if (strlen(maladie_nom) > 0) {
                                         mal *linkedMaladie = recherche_mal_nom(listeMaladies, maladie_nom);
                                         if (linkedMaladie != NULL) {
                                             nvPatient->maladie = linkedMaladie; // Liaison en mémoire
                                             printf("Maladie '%s' liée au patient '%s %s'.\n", linkedMaladie->nom, nvPatient->prenom, nvPatient->nom);
                                         } else {
                                             printf("Maladie '%s' non trouvée. Aucune maladie n'a été liée.\n", maladie_nom);
                                             nvPatient->maladie = NULL;
                                         }
                                    } else {
                                        printf("Aucune maladie n'a été saisie pour la liaison.\n");
                                        nvPatient->maladie = NULL;
                                    }
                                    ...
                                    // SAUVEGARDE DU PATIENT DANS LE FICHIER (après la liaison)
                                    savePatientToFile(nvPatient, patientFilePath);
                                }
                                else {
